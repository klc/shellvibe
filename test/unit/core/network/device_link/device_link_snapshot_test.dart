...
void main() {
  group('DeviceLinkSnapshot', () {
    ...
    test('keeps a wrapped row full width when its tail was erased', () {
      // Trailing cells are dropped from a serialized row, and the replay puts
      // no newline before a continuation — it lets the terminal wrap on its
      // own. A row that has a continuation must therefore be emitted at full
      // width, or the continuation starts wherever the trimmed row ended and
      // every row after it inherits the shift. Erasing to the end of a wrapped
      // row leaves exactly such blank-but-significant cells (a wide glyph that
      // does not fit at the right edge leaves one too).
      final source = Terminal(maxLines: 32);
      source.resize(5, 3);
      source.write('abcdefghij');
      // Back to the first row of that wrapped pair, erase from column 4 on.
      source.write('\x1b[1;4H\x1b[K');

      final snapshot = DeviceLinkSnapshot.capture(source);
      final destination = Terminal(maxLines: 32);
      snapshot.applyTo(destination);

      List<String> rendered(Terminal terminal) => terminal.buffer.lines
          .toList()
          .map((line) => line.toString().trimRight())
          .where((line) => line.isNotEmpty)
          .toList();

      expect(rendered(destination), rendered(source));
      expect(rendered(destination), ['abc', 'fghij']);
    });
