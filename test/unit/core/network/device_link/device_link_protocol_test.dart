...
void main() {
  group('DeviceLinkControlMessage', () {
    ...
    test('round-trips hello_ack session descriptors', () {
      ...
      expect(ack.sessions.single.title, 'zsh — ~/www/shellvibe');
    });
    ...
    test('round-trips a pair message with superseded device claims', () {
      final original = DeviceLinkPair(
        token: 'pairing-token',
        deviceName: 'Phone',
        devicePublicKey: 'phone-public-key',
        platform: 'ios',
        supersedes: const [
          DeviceLinkSupersededDevice(deviceId: 'old-1', secret: 'old-secret-1'),
          DeviceLinkSupersededDevice(deviceId: 'old-2', secret: 'old-secret-2'),
        ],
      );

      final decoded =
          DeviceLinkControlMessage.decode(original.encode()) as DeviceLinkPair;

      expect(decoded.supersedes, hasLength(2));
      expect(decoded.supersedes.first.deviceId, 'old-1');
      expect(decoded.supersedes.first.secret, 'old-secret-1');
      expect(decoded.toJson(), equals(original.toJson()));
    });

    test('omits supersedes from the wire when the phone claims nothing', () {
      final encoded = DeviceLinkPair(
        token: 'pairing-token',
        deviceName: 'Phone',
        devicePublicKey: 'phone-public-key',
      ).toJson();

      expect(encoded.containsKey('supersedes'), isFalse);
      ...
      expect(decoded.supersedes, isEmpty);
    });

    test('rejects malformed and oversized superseded claim lists', () {
      Map<String, Object?> pairWith(Object? supersedes) => {
        't': 'pair',
        'v': deviceLinkProtocolVersion,
        'token': 'pairing-token',
        'deviceName': 'Phone',
      ...
      expect(
        () => DeviceLinkControlMessage.fromJson(pairWith('old-1')),
      ...
      expect(
        () => DeviceLinkControlMessage.fromJson(
          pairWith(
            List.generate(deviceLinkMaxSupersededDevices + 1, (index) {
              return {'deviceId': 'old-$index', 'secret': 'secret-$index'};
            }),
          ),
        ),
        throwsA(isA<DeviceLinkProtocolException>()),
  ...
  group('DeviceLinkBinaryFrame', () {
    ...
    test('enforces outbound payload limits for every frame type', () {
      final limits = <DeviceLinkBinaryFrameType, int>{
        DeviceLinkBinaryFrameType.ptyInput: deviceLinkMaxPtyInputPayloadLength,
        DeviceLinkBinaryFrameType.ptyOutput:
            deviceLinkMaxPtyOutputPayloadLength,
        DeviceLinkBinaryFrameType.snapshot: deviceLinkMaxSnapshotPayloadLength,
      };

      for (final entry in limits.entries) {
        final atLimit = DeviceLinkBinaryFrame(
          type: entry.key,
          payload: List<int>.filled(entry.value, 0),
        ).encode();
        expect(atLimit.length, entry.value + 1);
        expect(atLimit.first, entry.key.prefix);

        expect(
          () => DeviceLinkBinaryFrame(
            type: entry.key,
            payload: List<int>.filled(entry.value + 1, 0),
          ).encode(),
          throwsA(
            predicate<DeviceLinkProtocolException>(
    ...
    test('rejects oversized control and binary frames before decoding', () {
      expect(
        () => DeviceLinkControlMessage.decode(
          '{"t":"hello","v":$deviceLinkProtocolVersion,"deviceId":"${'x' * deviceLinkMaxControlFrameLength}"}',
        ),
        throwsA(
          predicate<DeviceLinkProtocolException>(
            (error) => error.code == 'frame_too_large',
      ...
      expect(
        () => DeviceLinkBinaryFrame.decode(
          <int>[DeviceLinkBinaryFrameType.snapshot.prefix] +
              List<int>.filled(deviceLinkMaxSnapshotPayloadLength + 1, 0),
        ),
        throwsA(
          predicate<DeviceLinkProtocolException>(
            (error) => error.code == 'frame_too_large',
      ...
      expect(
        () => DeviceLinkBinaryFrame.decode(
          <int>[DeviceLinkBinaryFrameType.ptyInput.prefix] +
              List<int>.filled(deviceLinkMaxPtyInputPayloadLength + 1, 0),
        ),
        throwsA(
          predicate<DeviceLinkProtocolException>(
            (error) => error.code == 'frame_too_large',
    ...
    test('limits control frames by UTF-8 wire bytes', () {
      String encodedHello(int emojiCount) => jsonEncode(
        DeviceLinkHello(
          deviceId: '🙂' * emojiCount,
          deviceName: 'Phone',
        ).toJson(),
      ...
      var low = 0;
      var high = deviceLinkMaxControlFrameLength;
      while (low + 1 < high) {
        final middle = (low + high) ~/ 2;
        if (utf8.encode(encodedHello(middle)).length <=
            deviceLinkMaxControlFrameLength) {
          low = middle;
        } else {
          high = middle;
        }
      ...
      final belowLimit = encodedHello(low);
      final aboveLimit = encodedHello(high);
      expect(
        utf8.encode(belowLimit).length,
        lessThanOrEqualTo(deviceLinkMaxControlFrameLength),
      );
      expect(belowLimit.length, lessThan(deviceLinkMaxControlFrameLength));
      expect(
        DeviceLinkControlMessage.decode(belowLimit),
        isA<DeviceLinkHello>(),
      );
      expect(
        utf8.encode(aboveLimit).length,
        greaterThan(deviceLinkMaxControlFrameLength),
      );
      expect(
        () => DeviceLinkControlMessage.decode(aboveLimit),
        throwsA(
          predicate<DeviceLinkProtocolException>(
            (error) => error.code == 'frame_too_large',
          ),
