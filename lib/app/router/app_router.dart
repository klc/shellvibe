...
/// Navigator of the router itself.
///
/// Anything mounted through `ShadApp.router`'s `builder` — the MCP approval
/// host, for one — sits *above* the Navigator that builder wraps, so
/// `Navigator.of(context)` from there finds nothing and `showDialog` throws.
/// A key published here is the only handle such a widget has on the real
/// navigator.
...
final appRouterProvider = Provider<GoRouter>((ref) {
  ...
  return GoRouter(
    ...
    routes: [
      ...
      GoRoute(
        path: '/sftp',
        // Pushed over the shell, so it has to draw the shell's chrome itself
        // — without it the toolbar starts under the macOS traffic lights.
        builder: (context, state) => WindowChromeFrame(
          child: SftpDualPaneScreen(
