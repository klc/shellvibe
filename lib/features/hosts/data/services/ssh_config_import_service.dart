...
class SshConfigImportService {
  ...
  Future<SshConfigImportResult> importConfig({
    ...
    return db.transaction(() async {
      final existingHosts = await db.hostsDao.getHostsByWorkspace(
        options.workspaceId,
      );
      ...
      for (final draft in drafts) {
        ...
        if (conflicts &&
            options.conflictPolicy == SshConfigConflictPolicy.skip) {
          hostsSkipped++;
          continue;
        }
        ...
        if (hasKeyFiles && draft.identityFiles.isNotEmpty) {
          for (final keyPath in draft.identityFiles) {
            ...
            if (keyContent == null || keyContent.trim().isEmpty) {
              if (reportedIdentityIssuePaths.add(keyPath)) {
                warnings.add(
                  SshConfigWarning(
                    message:
                        'IdentityFile not readable: $keyPath '
                        '(host ${draft.alias})',
                  ),
                );
              }
            ...
            if (_looksLikePublicKey(keyContent)) {
              if (reportedIdentityIssuePaths.add(keyPath)) {
                warnings.add(
                  SshConfigWarning(
                    message:
                        'IdentityFile is a public key, not a private key: '
            ...
            if (_isEncryptedPrivateKey(keyContent)) {
              // Still imported — ShellVibe just cannot use it without a
              // passphrase prompt, which does not exist yet at connect time.
              warnings.add(
                SshConfigWarning(
                  message:
                      'IdentityFile $keyPath is passphrase-protected; '
                      'ShellVibe cannot decrypt it at connect time '
                      '(host ${draft.alias}).',
          ...
          if (options.conflictPolicy == SshConfigConflictPolicy.overwrite) {
            ...
            if (target == null) {
              ...
              warnings.add(
                SshConfigWarning(
                  message:
                      'Host "${draft.alias}" marked as existing but '
                      'not found; skipped.',
                ),
              );
              hostsSkipped++;
            ...
            if (options.importTunnels) {
              ...
              await (db.delete(
                db.portForwardRules,
              )..where((t) => t.hostId.equals(target.id))).go();
            }
      ...
      for (final draft in drafts) {
        ...
        if (targetId == null) {
          warnings.add(
            SshConfigWarning(
              message:
                  'ProxyJump target "${jump.host}" does not match any '
  ...
  /// decrypt. ShellVibe has no connect-time passphrase prompt, so such keys are
