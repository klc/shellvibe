...
class SshConfigResolver {
        ...
        if (insideStanza) {
          warnings.add(
            SshConfigWarning(
              source: d.source,
              line: d.line,
              message: 'Include inside a Host/Match block is not supported '
    ...
    for (final raw in d.args) {
      ...
      if (_hasGlobChars(expanded)) {
        ...
        final matches = entries
            .where((e) => _globMatch(pattern, e, classes: true))
            .toList()
          ..sort();
  ...
  ResolvedSshConfig _resolveAlias(
    ...
    void warn(SshConfigDirective d, String message,
        [SshConfigWarningSeverity severity =
            SshConfigWarningSeverity.warning]) {
      warnings.add(
        SshConfigWarning(
          source: d.source,
          line: d.line,
    ...
    for (final d in directives) {
      switch (d.keyword) {
        case 'host':
          if (d.args.isEmpty) {
            stanzaActive = false;
            warn(d, 'Host keyword requires at least one pattern; stanza '
                'ignored.');
          } else {
        ...
        case 'match':
          final evaluation = _evaluateMatch(d, alias, effectiveHostname(),
              effectiveUsername());
        ...
        default:
          ...
          switch (d.keyword) {
            ...
            case 'user':
              ...
              username =
                  _expandUserToken(d.args.first, alias, effectivePort(), d,
                      warn);
              break;
              ...
              if (tokens.isNotEmpty &&
                ...
                if (jump != null) {
                  ...
                  warn(
                    ...
                    '${jump.host}; Terly routes through the jump host at '
                    'connect time.',
                    SshConfigWarningSeverity.info,
                  );
                } else {
                  warn(d, 'ProxyCommand with -W but no usable jump host; '
                      'ignored.');
                }
              } else {
                warn(d, 'ProxyCommand is not supported by Terly; ignored.');
              }
              break;
            ...
            case 'dynamicforward':
              ...
              if (bind != null &&
                ...
                warn(
                  d,
                  'Forward bind address "$bind" is not preserved by Terly; '
            ...
            default:
              ...
              // Compression, …) has no Terly equivalent and is silently
              // dropped.
              break;
          }
      }
  ...
  static const _criterionKeywords = {
    ...
    'originalhost', 'tagged', 'command', 'user', 'localuser', 'version',
    'sessiontype',
  };
  ...
  ({bool active, SshConfigWarning? warning}) _evaluateMatch(
    ...
    while (i < args.length) {
      ...
      if (token == 'host' || token == 'originalhost' || token == 'user' ||
          token == 'localuser') {
        final list = <String>[];
        i++;
        while (i < args.length &&
        ...
        final patterns =
            list.expand((e) => e.split(',')).where((e) => e.isNotEmpty).toList();
        ...
        if (token == 'localuser') {
          return (
            ...
            warning: SshConfigWarning(
              source: d.source,
              line: d.line,
              message: 'Match localuser is not supported at import time; '
                  'stanza skipped.',
            ),
          );
        }
      ...
      return (
        ...
        warning: SshConfigWarning(
          source: d.source,
          line: d.line,
          message: 'Match $token cannot be evaluated at import time; '
              'stanza skipped.',
        ),
      );
    }
  ...
  String _expandTokens(
    ...
    for (var i = 0; i < value.length; i++) {
      ...
      switch (token) {
        ...
        default:
          warn?.call('Token %$token cannot be expanded at import time; '
              'left as-is.');
          buffer.write('%$token');
      }
    }
  ...
  /// chains (`host1,host2`) are cut to the first hop — Terly models a single
  ...
  ({SshForwardDraft? draft, String? reason}) _parseForward(
    ...
    if (type == 'dynamic') {
      ...
      if (listen == null) {
        return (
          draft: null,
          reason: 'Unparsable or unsupported listen spec '
    ...
    if (type == 'remote' && args.length == 1) {
      ...
      return (
        draft: null,
        reason: 'RemoteForward without a destination (remote SOCKS proxy) '
    ...
    if (dest == null) {
      return (
        draft: null,
        reason: 'Unparsable destination (${args[1]}); Unix sockets and '
