...
///
/// Measured, not guessed: the compact [IconButton] occupies 34px and the
/// [PopupMenuButton] 48px once Material's minimum tap target is applied, plus
/// the 12px the popup adds around its icon.
  ...
  Future<void> _deleteRunbook(RunbookModel runbook) async {
    final confirm = await showDialog<bool>(
      ...
      builder: (dialogContext) => ShadDialog.alert(
        ...
        description: Text('Are you sure you want to delete "${runbook.title}"?'),
        actions: [
          ShadButton.outline(
    ...
    return ListView(
      ...
      children: [
        ...
        TerlyStatusChip(
          ...
          tone: isExecuting ? TerlyStatusTone.warning : TerlyStatusTone.neutral,
        ),
        ...
        ShadButton(
          ...
          onPressed: onRun,
          child: Text(isExecuting ? 'Running…' : 'Run runbook'),
        ),
        ...
        const TerlySectionLabel(label: 'Steps', padding: EdgeInsets.zero),
        const SizedBox(height: 6),
        for (final step in runbook.steps)
          _RunbookStepTile(
            step: step,
        ...
        ShadButton.destructive(
          ...
          width: double.infinity,
          onPressed: onDelete,
          child: const Text('Delete runbook'),
        ),
      ],
    );
  }
