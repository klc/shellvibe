...
/// [second]; a draggable divider sits between them — 10px wide under a pointer,
/// 22px under a thumb, with a 2px visual
...
class ResizableSplit extends StatefulWidget {
  ...
  /// Grab width of the divider. The visual rule stays 2px either way; this is
  /// only how much of the gutter accepts the drag, and a thumb needs more of it
  /// than a pointer does.
  static double get _hit => isMobilePlatform ? 22 : 10;
...
class _ResizableSplitState extends State<ResizableSplit> {
  ...
  void _handleUpdate(double delta, double total) {
    ...
    setState(() {
      _dragRatio = (_startRatio - _dragTotal / total).clamp(
        kSplitPaneMinRatio,
        kSplitPaneMaxRatio,
      );
    });
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        final secondFlex = (_ratio * ResizableSplit._flexUnits).round().clamp(
          1,
          ResizableSplit._flexUnits - 1,
        );
