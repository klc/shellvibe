...
class VaultRepository {
  ...
  /// Ids of identities whose stored secrets cannot be decrypted with the
  /// current vault key.
  ///
  /// Exists because listing identities deliberately skips decryption, which
  /// leaves a broken identity looking exactly like a healthy one until a
  /// connection fails. Plaintext is produced and discarded rather than
  /// returned: an AES-GCM tag cannot be verified without decrypting what it
  /// protects, but nothing outside this method ever sees the result.
  ///
  /// Returns an empty set when the vault is locked — nothing can be judged
  /// unreadable while the key to read it is unavailable.
  Future<Set<String>> findUndecryptableIdentityIds({
    String? workspaceId,
  }) async {
    ...
    for (final row in rows) {
      for (final ciphertext in [
        ...
        try {
          ...
          // Any failure to read it back — a MAC mismatch from a different vault
          // key, or a value that is not well-formed ciphertext at all — means
          // this identity needs repairing.
          broken.add(row.id);
          break;
        }
      }
