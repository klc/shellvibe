...
class RemoteFileEditorDialog extends ConsumerStatefulWidget {
  ...
  const RemoteFileEditorDialog({super.key, required this.fileItem});

  @override
  ConsumerState<RemoteFileEditorDialog> createState() =>
      _RemoteFileEditorDialogState();
}

class _RemoteFileEditorDialogState
    extends ConsumerState<RemoteFileEditorDialog> {
  ...
  Future<void> _loadFileContent() async {
    ...
    try {
      final content = await ref
          ...
          .readRemoteFileContent(widget.fileItem.path);
      if (mounted) {
  ...
  Future<void> _saveFileContent() async {
    ...
    try {
      ...
      if (mounted) {
        ...
        ShadToaster.of(
          context,
        ).show(const ShadToast(title: Text('File saved successfully')));
      }
    } catch (e) {
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    ...
    return PopScope(
      ...
      child: ShadDialog(
        ...
        actions: [
          ShellVibeButton.secondary(label: 'Cancel', onPressed: _handleClose),
          if (_isSaving)
            const SizedBox(
              width: 20,
              height: 20,
