...
class RunbooksDao extends DatabaseAccessor<AppDatabase> with _$RunbooksDaoMixin {
  ...
  Future<int> insertRunbook(RunbooksCompanion runbook) => into(runbooks).insert(runbook);

  Future<bool> updateRunbook(RunbooksCompanion runbook) => update(runbooks).replace(runbook);
  ...
  Future<void> replaceSteps(String runbookId, List<RunbookStepsCompanion> steps) async {
    await transaction(() async {
      await (delete(runbookSteps)..where((tbl) => tbl.runbookId.equals(runbookId))).go();
