...
class KnownHostsDao extends DatabaseAccessor<AppDatabase>
    with _$KnownHostsDaoMixin {
  KnownHostsDao(super.db);

  Future<KnownHost?> findKnownHost(String hostname, int port) async {
    final list =
        await (select(knownHosts)..where(
              (tbl) => tbl.hostname.equals(hostname) & tbl.port.equals(port),
            ))
            .get();
